...
void DynamicCache::init_cache(int64_t nodes_per_gpu, int32_t feature_len, 
    ...
    int **host_cache_lru_ptr = (int**)malloc(num_gpus * sizeof(int*));
    ...
    cudaCheckError();
    cudaMalloc(&dev_cache_lru, num_gpus * sizeof(int*));
    cudaCheckError();
    ...
    for(int i = 0; i < num_gpus; i++) {
        ...
        cudaMalloc(&host_cache_ptr[i], num_sets * num_ways * feature_len * sizeof(float));
        cudaCheckError();
        cudaMalloc(&host_cache_lru_ptr[i], num_sets * num_ways * sizeof(int));
        cudaCheckError();
        cudaMalloc(&host_cache_key_ptr[i], num_sets * num_ways * sizeof(int));
        cudaCheckError();
        reset_cache_metadata<<<32, 512>>>(host_cache_key_ptr[i], host_cache_lru_ptr[i], num_sets * num_ways);
        cudaCheckError();
    }
    // Transfer pointers to GPU
    cudaMemcpy(dev_cache_storage, host_cache_ptr, num_gpus * sizeof(void*), cudaMemcpyHostToDevice);
    cudaCheckError();
    cudaMemcpy(dev_cache_lru, host_cache_lru_ptr, num_gpus * sizeof(int*), cudaMemcpyHostToDevice);
    cudaCheckError();
    ...
    int *dev_tracker;
    cudaMalloc(&dev_tracker, sizeof(int));
...
/*  
    ...
    test_lookup_features(num_nodes, cpu_features, index_array, dev_tracker);
    cudaCheckError();

    int success_searches;
    cudaMemcpy(&success_searches, dev_tracker, sizeof(int), cudaMemcpyDeviceToHost);
    std::cout << "Successful searches: " << success_searches << " out of " << num_nodes << "\n"; 
*/
...
void DynamicCache::test_lookup_features(int64_t num_nodes, float *input_feats, 
    int32_t *index_array, int *success_lookups)
{
    test_lookup_features_kernel<<<32, 512>>>(
        dev_cache_storage, dev_cache_key, dev_cache_lru, num_nodes, feature_len, 
        input_feats, index_array, num_gpus, num_ways, num_sets, success_lookups);
}